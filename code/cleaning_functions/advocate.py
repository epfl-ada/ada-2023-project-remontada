...
def clean_advocate_users(df_advocate_users):
    ...
    if df_advocate_users["joined"].dtype != "datetime64[ns]":
        df_advocate_users["joined"] = pd.to_datetime(
            pd.to_numeric(df_advocate_users["joined"]), unit="s"
        )
...
def clean_advocate_ratings(df_advocate_ratings, df_all_beers):
    ...
    cols_to_convert_bool = ["review"]
    cols_to_convert_name = ["beer_id", "brewery_id"]

    df_advocate_ratings[cols_to_convert_float] = df_advocate_ratings[
        cols_to_convert_float
    ].apply(pd.to_numeric, errors="coerce")
    df_advocate_ratings[cols_to_convert_str] = df_advocate_ratings[
        cols_to_convert_str
    ].astype(str)
    df_advocate_ratings[cols_to_convert_name] = (
        df_advocate_ratings[cols_to_convert_name]
        .astype(str)
        .apply(lambda x: x.radd("ba_"))
    )
    df_advocate_ratings[cols_to_convert_bool] = df_advocate_ratings[
        cols_to_convert_bool
    ].astype(bool)
    df_advocate_ratings["date"] = pd.to_datetime(
        pd.to_numeric(df_advocate_ratings["date"]), unit="s"
    )

    # Replace beer_id with unique_id from df_all_beers
    merged_df = df_advocate_ratings.merge(
        df_all_beers[["ba_beer_id", "beer_unique_id"]],
        left_on="beer_id",
        right_on="ba_beer_id",
        how="left",
    )
...
def clean_advocate_beers(df_advocate_beers):
    ...
    df_advocate_beers[cols_to_convert_name] = (
        df_advocate_beers[cols_to_convert_name].astype(str).apply(lambda x: "ba_" + x)
    )

    # drop duplicates
    df_advocate_beers.drop_duplicates(
        subset=["ba_beer_id", "ba_brewery_id"], inplace=True
    )
...
def clean_advocate_reviews(df_advocate_reviews):
    ...
    cols_to_convert_str = ["text", "user_id"]

    # Keep columns of interest and convert types
    df_advocate_reviews = df_advocate_reviews[cols_of_interest]

    df_advocate_reviews[cols_to_convert_str] = df_advocate_reviews[
        cols_to_convert_str
    ].astype(str)
    df_advocate_reviews["date"] = pd.to_datetime(
        pd.to_numeric(df_advocate_reviews["date"]), unit="s"
    )

    # Correct wrong character
    df_advocate_reviews["text"] = df_advocate_reviews["text"].apply(
        lambda x: fix_text(x)
    )
