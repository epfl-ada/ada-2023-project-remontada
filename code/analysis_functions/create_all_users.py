...
def create_all_users(df_advocate_users, df_matched_beer_users, df_rate_beer_users):
    ...
    if os.path.exists(path + file):
        ...
        rate_beer = rate_beer[
            ["rb_user_id", "rb_user_name", "rb_joined", "rb_location"]
        ]
        matched = matched[
            [
                "ba_joined",
        ...
        df_datasets_users = pd.concat(
            [df_dataset_users_advocate, df_dataset_users_rate_beer], axis=0
        )

        # Wrap duplicate values to a single column
        df_datasets_users["user_id"] = df_datasets_users["ba_user_id"].fillna(
            df_datasets_users["rb_user_id"]
        )
        df_datasets_users["location"] = df_datasets_users["ba_location"].fillna(
            df_datasets_users["rb_location"]
        )
        df_datasets_users["joined"] = df_datasets_users["ba_joined"].fillna(
            df_datasets_users["rb_joined"]
        )

        # drop unnecessary columns
        df_datasets_users.drop(
            columns=[
                "ba_user_id",
                "ba_location",
                "ba_joined",
