...
def create_ratings_stat(
    df_advocate_ratings, df_rate_beer_ratings, df_all_users, read_pickle_if_exists=True
):
    ...
    if os.path.exists(path + file) and read_pickle_if_exists:
        ...
        df_advocate_ratings_stats.drop(
            df_advocate_ratings_stats[df_advocate_ratings_stats.user_id.isna()].index,
            inplace=True,
        )
        df_rate_beer_ratings_stats.drop(
            df_rate_beer_ratings_stats[df_rate_beer_ratings_stats.user_id.isna()].index,
            inplace=True,
        )
        df_advocate_ratings_stats.rename(
            columns={"user_name": "ba_user_name"}, inplace=True
        )
        df_rate_beer_ratings_stats.rename(
        ...
        ratings_stats = deepcopy(
            df_advocate_ratings_stats.merge(df_all_users, how="inner")
        )
        df_rate_beer_ratings_stats.user_id = df_rate_beer_ratings_stats.user_id.astype(
            int
        )
        df_rate_beer_ratings_stats.user_id = df_rate_beer_ratings_stats.user_id.astype(
            object
        ...
        ratings_stats2 = deepcopy(
            df_rate_beer_ratings_stats.merge(df_all_users, how="inner")
        )

        # Concatenate the dataframes
        print("Concatenating dataframes...")
        ratings_stats = pd.concat(
            [ratings_stats, ratings_stats2], axis=0, keys=["ba", "rb"]
        )
