...
def compute_expert_score(
    row, weight_y=2, weight_y_1=0.5, weight_y_2=0.25, weight_y_3=0.1
):
...
def compute_experts_table(df_ratings, quantile_score_expert=0.995):
    ...
    df_user_ratings_per_year = (
        df_ratings.groupby(["user_id", "year"]).size().reset_index(name="nb_ratings")
    )
    df_ratings_stat_pivot = df_user_ratings_per_year.pivot(
        index="user_id", columns="year", values="nb_ratings"
    )
    ...
    df_ratings_stat_pivot = df_ratings_stat_pivot.reindex(
        columns=[df_ratings_stat_pivot.columns[0]]
        + [1997]
        + list(df_ratings_stat_pivot.columns[1:-1])
    )
    df_ratings_stat_pivot_score = df_ratings_stat_pivot.apply(
        compute_expert_score, axis=1
    )

    #     # Compute the threshold to be an expert
    df_ratings_stat_pivot_expert = df_ratings_stat_pivot_score.gt(
        df_ratings_stat_pivot_score[df_ratings_stat_pivot_score.gt(0)].quantile(
            quantile_score_expert
        )
    )
    ...
    df_melted = pd.melt(
        df_reset, id_vars=["user_id"], var_name="year", value_name="is_expert"
    )
...
def filter_year_and_add_is_expert(df, YEAR, experts_table):
    ...
    experts_year = (
        experts_table[
            (experts_table["year"] == YEAR) & (experts_table["is_expert"] == True)
        ]
        .user_id.values.astype(str)
        .tolist()
    )
