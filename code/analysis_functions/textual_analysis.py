...
def create_text_dataset(
    df_advocate_reviews, df_rate_beer_reviews, df_all_users, experts_dict
):
    ...
    if os.path.exists(path + file):
        ...
        df_advocate_reviews_stats.rename(
            columns={"user_name": "ba_user_name"}, inplace=True
        )
        df_rate_beer_reviews_stats.rename(
            columns={"user_name": "rb_user_name"}, inplace=True
        )

        # Merge the dataframes
        print("Merging dataframes...")
        ratings_stats = deepcopy(
            df_advocate_reviews_stats.merge(df_all_users, how="inner")
        )
        ratings_stats["user_id"] = ratings_stats["user_id"].rename("ba_user_id")
        df_rate_beer_reviews_stats.user_id = df_rate_beer_reviews_stats.user_id.astype(
            int
        )
        df_rate_beer_reviews_stats.user_id = df_rate_beer_reviews_stats.user_id.astype(
            object
        )
        ratings_stats2 = deepcopy(
            df_rate_beer_reviews_stats.merge(
                df_all_users, how="inner", on="rb_user_name"
            )
        )
        ratings_stats2["user_id"] = ratings_stats2["user_id_y"].fillna(
            ratings_stats2["user_id_x"]
        )
        ...
        # Order the columns
        ordered_cols = ["year", "location", "user_id", "text", "is_expert"]
...
def compute_text_stats(df_texts):
    ...
    if df_texts_casual is None or df_texts_expert is None:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            df_texts["language"] = list(
                tqdm(
                    executor.map(detect_language_wrapper, df_texts["text"]),
                    total=len(df_texts),
                )
            )

            df_texts["nb_words"] = list(
                tqdm(
                    executor.map(calculate_nb_words_wrapper, df_texts["text"]),
                    total=len(df_texts),
                )
            )
        ...
        df_texts_expert["nb_reviews"] = df_texts_expert.groupby("user_id")[
            "user_id"
        ].transform("size")
        df_texts_casual["nb_reviews"] = df_texts_casual.groupby("user_id")[
            "user_id"
        ].transform("size")
...
def compute_top_words_adjectives(df):
    ...
    words = Counter(
        [
            item
            for sublist in tqdm(df["tokens"], desc="Processing tokens")
            for item in sublist
        ]
    )
    ...
    adj = []
    count_adj = []
    for top_word, count_word in zip(
        top_words["common_words"], top_words["count_words"]
    ):
...
def compute_top_words_tfidf(docs, topk=20):
    ...
    word_tfidf_list = pd.DataFrame(
        {
            "words": words,
            "words_score": words_score,
            "adjs": adjs,
            "adjs_score": adjs_score,
        }
    )
...
def tokenize(df_texts):
    ...
    nlp = spacy.load("en_core_web_sm")
    with concurrent.futures.ThreadPoolExecutor() as executor:
        df_texts["tokens"] = list(
            tqdm(
                executor.map(tokenize_texts, df_texts["text"], [nlp] * len(df_texts)),
                total=len(df_texts),
            )
        )
    return df_texts
...
def add_bigrams(df_texts):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        df_texts["bigrams"] = list(
            tqdm(executor.map(compute_bigrams, df_texts["tokens"]), total=len(df_texts))
        )
    return df_texts
...
def sentiment(df_texts_casual_sentiment, df_texts_expert_sentiment):
    # Compute the statistics
    if os.path.exists(
        "../data/processed/df_texts_casual_sentiment.pkl"
    ) and os.path.exists("../data/processed/df_texts_expert_sentiment.pkl"):
        print("Loading the dataframe in pickle format from ../data/processed/")
        df_texts_casual_sentiment = load_pickle(
            "../data/processed/df_texts_casual_sentiment.pkl"
        )
        df_texts_expert_sentiment = load_pickle(
            "../data/processed/df_texts_expert_sentiment.pkl"
        )
    else:
        print("Analyzing sentiment...")
        with concurrent.futures.ThreadPoolExecutor() as executor:
            df_texts_casual_sentiment["sentiment"] = list(
                tqdm(
                    executor.map(
                        analyze_sentiment_wrapper, df_texts_casual_sentiment["text"]
                    ),
                    total=len(df_texts_casual_sentiment),
                )
            )
            # Extract sentiment scores into separate columns
            df_texts_casual_sentiment[
                ["Neg_sentiment", "Neu_sentiment", "Pos_sentiment", "Comp_sentiment"]
            ] = pd.DataFrame(
                df_texts_casual_sentiment["sentiment"].tolist(),
                index=df_texts_casual_sentiment.index,
            )

            df_texts_expert_sentiment["sentiment"] = list(
                tqdm(
                    executor.map(
                        analyze_sentiment_wrapper, df_texts_expert_sentiment["text"]
                    ),
                    total=len(df_texts_expert_sentiment),
                )
            )
            # Extract sentiment scores into separate columns
            df_texts_expert_sentiment[
                ["Neg_sentiment", "Neu_sentiment", "Pos_sentiment", "Comp_sentiment"]
            ] = pd.DataFrame(
                df_texts_expert_sentiment["sentiment"].tolist(),
                index=df_texts_expert_sentiment.index,
            )
